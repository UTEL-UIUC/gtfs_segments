...
class TestGTFSSegments(unittest.TestCase):
    ...
    def test_plot_histogram(self):
        ...
        self.assertTrue(type(fig) == matplotlib.figure.Figure, "Error with plot_hist. Should work for the SFMTA example feed")
        fig = plot_hist(self.df, show_mean=True, save_fig =True, file_path = 'test_hist.png')
        self.assertTrue(type(fig) == matplotlib.figure.Figure, "Error with plot_hist. Should work for the SFMTA example feed")
        self.assertTrue(os.path.exists('test_hist.png'), 'Check if the test_hist.png file exists')
                
    def test_summary_stats(self):
        summ_df = summary_stats(self.df)
        self.assertTrue(type(summ_df) == pd.DataFrame, "Error with summary_stats. Result should be a DataFrame")
        summ_df = summary_stats(self.df, export = True, file_path = 'test_summary_stats.csv')
        self.assertTrue(type(summ_df) == pd.DataFrame, "Error with summary_stats. Result should be a DataFrame")
    ...
    def test_export_segments(self):
        ...
        self.assertTrue(len(df) == len(self.df), "Error with export_segments. Should work for the SFMTA example feed")
