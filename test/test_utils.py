...
class TestGTFSSegments(unittest.TestCase):
    ...
    def test_plot_histogram(self):
        ...
        fig = plot_hist(
            self.df, show_mean=True, save_fig=True, file_path="test_hist.png"
        )
        self.assertTrue(
            type(fig) == matplotlib.figure.Figure,
            "Error with plot_hist. Should work for the SFMTA example feed",
        ...
        self.assertTrue(
            os.path.exists("test_hist.png"), "Check if the test_hist.png file exists"
        )
    ...
    def test_summary_stats(self):
        ...
        summ_df = summary_stats(
            self.df, export=True, file_path="test_summary_stats.csv"
        )
        self.assertTrue(
        ...
        self.assertTrue(
            os.path.exists("test_summary_stats.csv"),
    ...
    def test_export_segments(self):
        ...
        export_segments(
            self.df, "test_export_segments.csv", output_format="csv", geometry=False
        )
        self.assertTrue(
            os.path.exists("test_export_segments.csv"),
            "Check if the test_export_segments.csv file exists",
        ...
        self.assertTrue(
            "geometry" not in df.columns, "Check if the geometry column is not present"
        )

        # Test export_segments for .geojson and with geometry
        export_segments(
            self.df, "test_export_segments.geojson", output_format="geojson"
        )
        self.assertTrue(
            os.path.exists("test_export_segments.geojson"),
